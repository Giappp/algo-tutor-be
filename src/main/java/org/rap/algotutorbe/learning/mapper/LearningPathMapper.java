...
public interface LearningPathMapper {
    ...
    LearningPath toEntity(CreateOrUpdateLearningPathRequest request);

    @Mapping(target = "id", ignore = true)
    @Mapping(target = "slug", expression = "java(slugGenerator.generateFrom(request.name()))")
    void updateEntity(@MappingTarget LearningPath entity, CreateOrUpdateLearningPathRequest request);
}
